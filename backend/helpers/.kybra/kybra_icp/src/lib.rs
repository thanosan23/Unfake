...
#[ic_cdk_macros::query(name = "get_chain")]
#[candid::candid_method(query, rename = "get_chain")]
async fn _cdk_user_defined_get_chain() -> (String) {
    let interpreter = unsafe { INTERPRETER_OPTION.as_mut() }
        .unwrap_or_trap("SystemError: missing python interpreter");
    let vm = &interpreter.vm;
    let params = ();
    call_global_python_function("get_chain", params)
        .await
        .unwrap_or_trap()
}
#[ic_cdk_macros::update(name = "add_block")]
#[candid::candid_method(update, rename = "add_block")]
async fn _cdk_user_defined_add_block(_cdk_user_defined_block: Block) -> () {
    let interpreter = unsafe { INTERPRETER_OPTION.as_mut() }
        .unwrap_or_trap("SystemError: missing python interpreter");
    let vm = &interpreter.vm;
    let params = (_cdk_user_defined_block
        .try_into_vm_value(vm)
        .unwrap_or_trap(),);
    call_global_python_function("add_block", params)
        .await
        .unwrap_or_trap()
}
#[derive(
...
struct Block {
    id: Box<String>,
    qr_code: Box<QR>,
    productName: Box<String>,
    hashes: Box<String>,
    previous_hash: Box<String>,
    nonce: Box<candid::Int>,
    timestamp: Box<String>,
}
#[derive(
    serde :: Deserialize,
    Debug,
